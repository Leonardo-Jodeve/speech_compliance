...
const state = {view:"calls", calls:[], selected:new Set(), searchId:null, callPage:1, callQuery:null, adminPage:1, taskPage:1, adminRows:[], editing:null, deleting:null, detailId:null, polling:null};
...
function navigate(view) {
  ...
  if (view === "tasks") loadTasks();
    ...
    state.adminPage = 1; $("#admin-search").value = "";
    $("#admin-title").textContent = titles[view];
    $("#admin-help").textContent = {scenes:"场景 ID 必须与源通话 act_scene_id 一致。停用场景后，该场景不再加载规则。删除前须先移除场景规则关联。",rules:"按通话发生时间匹配规则生效区间（开始包含、结束不含）。致命规则不通过会直接判定不合规。修改规则不会重跑已完成任务。",bindings:"先创建场景和规则，再建立关联。权重覆盖留空时使用规则默认权重；填 0 表示该场景下权重为 0。只有场景、规则和关联都启用时才参与评判。"}[view];
    loadAdmin();
  }
}
...
async function loadAdmin() {
  ...
  const headers = {scenes:["场景名称 / 源场景 ID","场景描述","状态","更新时间","操作"],rules:["规则名称 / 编码","规则类型","严重程度 / 权重","生效区间","状态","操作"],bindings:["营销场景","话术规则","场景权重","状态","操作"]};
...
async function saveEditor(event) {
  ...
  try {
    ...
    if (state.view === resource) loadAdmin();
  } catch(error) { $("#editor-error").textContent = error.message; }
  finally { button.disabled = false; }
}
...
async function deleteRecord() {
  ...
  try { await api(`/api/admin/${resource}/${id}`,{method:"DELETE"}); $("#delete-dialog").close(); toast("配置已删除"); loadAdmin(); }
